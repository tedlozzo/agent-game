...
test_model() {
    ...
    case "$tool" in
        claude)
            ...
            ;;
        cursor)
            # Test Cursor model with same mode as benchmark (CURSOR_MODE + CURSOR_AGENT_MODE_MODELS)
            ...
            local mode_args=()
            [ "$cursor_mode" = "ask" ] && mode_args=("--mode" "ask")
            if timeout 30s "$cli" agent -p --trust --model "$model" "${mode_args[@]}" "Say 'OK'" < /dev/null > /tmp/model_test_$$.txt 2>&1; then
                if grep -q "OK\|ok" /tmp/model_test_$$.txt; then
                    echo "✓ OK"
                    rm -f /tmp/model_test_$$.txt
                    return 0
